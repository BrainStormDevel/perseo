...
$wizardMiddleware = function (Request $request, Response $response, callable $next) use ($container) {
	...
	if (\PerSeo\CheckConfig::verify() && !in_array($routeName, $publicRoutesArray)) { $response = $response->withRedirect($uri .'/wizard'); }
...
$adminMiddleware = function (Request $request, Response $response, callable $next) use ($container) {
	...
	$uri = $request->getUri()->getBasePath();
	if (!$login->islogged('admins') && in_array($routeName, $publicRoutesArray)) { $response = $response->withRedirect($uri .'/login/admin'); }
	else { $response = $next($request, $response); }
    return $response;
};
