...
function basicshutdownFunction() {
    ...
    if ($error !== null) {
        ...
        $logname = 'error_'.date('d-m-Y').'.log';
        $logdate = date('d/m/Y h:i:a');
        $text = "Error: {$error['message']} in {$error['file']} on line {$error['line']}";
        file_put_contents($logdir . DIRECTORY_SEPARATOR . $logname, "$logdate $text \n", FILE_APPEND);
